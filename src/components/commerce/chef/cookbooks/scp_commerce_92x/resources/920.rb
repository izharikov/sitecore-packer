...
action :prepare do
  ...
  powershell_script 'Install Redis' do
    code <<-EOH
  ...
  scp_sitecore_common_download_sitecore_package 'Sitecore Commerce 9.2.0' do
    package_url install['commerce_package_url']
    package_path install['commerce_package_zip_path']
    user secrets['user']
    password secrets['password']
  end
  scp_sitecore_common_unzip 'Sitecore Commerce 9.2.0' do
